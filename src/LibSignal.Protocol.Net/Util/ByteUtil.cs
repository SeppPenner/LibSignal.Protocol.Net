namespace LibSignal.Protocol.Net.Util
{
    ...
    public class ByteUtil
    {
        ...
        {
            if (input == null || firstLength < 0 || secondLength < 0 || thirdLength < 0 || input.Length < firstLength + secondLength + thirdLength)
            {
        ...
        {
            return ((bytes[offset] & 0xffL) << 32) | ((bytes[offset + 1] & 0xffL) << 24) | ((bytes[offset + 2] & 0xffL) << 16) | ((bytes[offset + 3] & 0xffL) << 8) | ((bytes[offset + 4] & 0xffL));
        }

        public static long byteArrayToLong(byte[] bytes, int offset)
        {
            return ((bytes[offset] & 0xffL) << 56) | ((bytes[offset + 1] & 0xffL) << 48) | ((bytes[offset + 2] & 0xffL) << 40) | ((bytes[offset + 3] & 0xffL) << 32) | ((bytes[offset + 4] & 0xffL) << 24) | ((bytes[offset + 5] & 0xffL) << 16)
